...
function newDragonTemplate(){
    var
        dragon_name_str = 'new dragon',
        description_str = 'description',
        damage_int = '',
        protection_int = '',
        html_str = '';

    html_str += '<section data-role="card_wrapper_new">';
    ...
    // html_str += 		'<label>image</label>';
    html_str += 		'<input data-image="file" type="file" id="file" multiple >';
    ...
    html_str += 			'<svg xmlns="http://www.w3.org/2000/svg" width="35" height="35" viewBox="0 1 20 20" id="IconChangeColor"> <path d="M7 2a.5.5 0 0 1 .5.5v5h5a.5.5 0 0 1 0 1h-5v5a.5.5 0 0 1-1 0v-5h-5a.5.5 0 0 1 0-1h5v-5A.5.5 0 0 1 8 2Z"></path> </svg>';
    html_str += 		'</span>';
    html_str += 		'<section data-role="card_internals">';
    html_str += 			'<h4 data-role="dragon_name">' + 'Create new' + '</h4>';
    html_str += 			'<span data-role="damage">' + '?' + '</span>';
    html_str += 			'<span data-role="protection">' + '?' + '</span>';
    html_str += 			'<figure>';
    html_str += 			'<img alt="This is a picture of ' +  dragon_name_str + ' " id="base64Img" src="" width="300" height="300" />'; 
    
    html_str += 				'<figcaption data-role="description">' + 'describe it ...' + '</figcaption>';
    html_str += 			'</figure>';
    html_str += 		'</section>';
    html_str += 	'</article>';
    html_str += '</section>';
...
function saveEditDragon(){
	...
	var 
		$this = $(this),
		params = constructEditPayload($this),
        creating = false;
    
    // this part is for new dragon creation
    if($this.parent().parent().attr('data-role') === 'card_wrapper_new'){
        creating = true;
        var
            file = $('#file')[0]['files'][0],
            reader = new FileReader();
        params.new_dragon = true;
        params.original_dragon_name_str = params.updates.dragon_name_str;
        // From here on, payload will be string type so it's easier to add base64 string encoding of images to the payload. The content-type will be "text/plain".
        params = JSON.stringify(params);
        reader.onloadend = function() {
            $("#base64Img").attr("src", reader.result);
            params = params.slice(0,-1) + ', "image": "' + reader.result.slice(22) + '"}';
            console.log('image saved!');
            jQuery.ajax({
                url: window.API_ENDPOINT_URL_STR + "/create",
                type: "POST",
                data: params,
                headers: {
                ...
                success: function(data){
                    successfulEdit($this, data, creating);
                },
                fail: failEdit,
                timeout: 3000
            });
...
function successfulEdit($this, data, creating){
	var
		$dragon_article = $this.parent().parent().find("[data-face='back']"),
		$other = $this.parent().parent().find("[data-face='front']");
    
    // console.log('Here is the return data');
    // console.log(data);

    if(creating === true){
        $this.parent().parent().attr('data-role', 'card_wrapper');
        // change icon
        $('svg path').attr('d', "M3 17.25V21h3.75L17.81 9.94l-3.75-3.75L3 17.25zM20.71 7.04c.39-.39.39-1.02 0-1.41l-2.34-2.34c-.39-.39-1.02-.39-1.41 0l-1.83 1.83 3.75 3.75 1.83-1.83z");
        $('svg').attr({'width':"24",
                   'height': "24",
                    'viewBox': "0 0 24 24"});
        $('svg').append('<path d="M0 0h24v24H0z" fill="none"/>');
        // add new template for further creation
        $dragon_info.append(newDragonTemplate());
    }
...
function constructEditPayload($this){
    ...
    console.log('payload is being constructed');
